function createProjectElement(project, projectRepository, updateCallback) {
    const newProjectLi = document.createElement("li");

    const progressAndNameContainer = document.createElement("div");
    progressAndNameContainer.classList.add("progress-and-name-container");
...
function updateProjectNav(projectRepository) {
    ...
    activeProjects.forEach((project) => {
        const newProjectElement = createProjectElement(
            project,
            projectRepository,
            updateProjectNav // Pass the update function so the delete button can trigger a re-render
        );
        projectsContainer.appendChild(newProjectElement);
    });
}

export { updateProjectNav };
